...
\def\ivoaBaseURL{http://ivoa.net/documents/} % URL de base attendue
\def\ivoaDocdatecode{20250627} % Par exemple, date au format YYYYMMDD
\def\ivoaDocname{SLAP2}
\def\ivoaDocversion{1.0}
\def\ivoaDocdate{2025-06-17}
\def\ivoaDocauthors{Nicolas Moreau} % Remplacez par les auteurs
\def\ivoaDocstatus{WD} % ou "WD", "REC", "NOTE", etc.
\def\ivoaDocid{IVOA-SLAP2-1.0}
\def\ivoaDoctype{WD} % ou "Working Draft", "Recommendation", etc.
...
(some of which may be custom to that particular service) to control more finely the selection of spectral lines. Fully compliant services will
also provide the \{species\} resource, returning a list of
all the species for which lines are available.\textbf{\textbf{}}
\end{abstract}
...
clients will find their paths thanks to the VOSI-capabilities
...
of values with optional lower and upper bounds separated by a / (slash) character. 
If  the lower or upper bound is not specified, the range is open-ended. \\ \\
...
The units for numeric values are specified for each parameter and never included in the value. If the separator (/) is not present, the interval is assumed to be a scalar value.
...
The service \textbf{SHOULD} support the MAXREC parameter defined in DALI.
...
A service \textbf{MAY} implement the parameters \textbf{SPECIES\_MASS} to specify the minimum and maximum mass of a species, 
to be specified in Unified Atomic Mass Unit (u).  
...
A service \textbf{MAY} implement the parameters \textbf{ION\_CHARGE} to specify the minimum and maximum charge of an ion (ucd={\textquotedbl}phys.atmol.ionization{\textquotedbl}; 
...
\parexample{CHEMICAL\_ELEMENT=Fe\&ION\_CHARGE=1/1}
...
A service \textbf{MAY} implement the parameters \textbf{LOWER\_LEVEL\_ENERGY}
...
A service \textbf{MAY}implement the parameters
\textbf{UPPER\_LEVEL\_ENERGY} to specify the minimum 
...
A service \textbf{MAY} implement the parameters \textbf{TEMPERATURE} to
specify the minimum and maximum expected temperatures of the object, to
be specified in Kelvin (ucd={\textquotedbl}phys.temperature{\textquotedbl};corresponding VAMDC restrictable is {\textquotedbl}EnvironmentTemperature{\textquotedbl}). This parameter would be used (in particular for
...
A service \textbf{MAY }implement the parameter \textbf{EINSTEINA} to accept constraints in the transition probability by specifying the minimum and maximum Einstein A, defined as the probability per unit time s\textsuperscript{-1} for spontaneous emission in a bound-bound transition (ucd={\textquotedbl}phys.atmol.transProb{\textquotedbl};corresponding VAMDC restrictable is {\textquotedbl}RadTransProbabilityA{\textquotedbl})..
...
A standard column could appear multiple times with different units. 
...
\subsubsection{Vacuum wavelength}

Exactly one field \textbf{MUST} have name={\textquotedbl}vacuum\_wavelength{\textquotedbl},
with datatype={\textquotedbl}double{\textquotedbl}, unit={\textquotedbl}m{\textquotedbl} and
ucd={\textquotedbl}em.wl{\textquotedbl}, containing the wavelength in
vacuum of the transition originating the line in meters. The corresponding VAMDC returnable is  {\textquotedbl}\textbf{RadTransWavelength{\textquotedbl}}.

\subsubsection{Line title}

Exactly one field \textbf{MUST} have name={\textquotedbl}line\_title{\textquotedbl}, with
datatype={\textquotedbl}char{\textquotedbl}, arraysize={\textquotedbl}*{\textquotedbl} and ucd={\textquotedbl}meta.title{\textquotedbl}, containing a small description identifying the line.
The corresponding VAMDC returnable is {\textquotedbl}\textbf{RadTransComment}{\textquotedbl}.
...
\subsubsection{Chemical element name}

Exactly one field \textbf{SHOULD} have 
name={\textquotedbl}chemical\_element\_name{\textquotedbl}, with  datatype={\textquotedbl}char{\textquotedbl}, arraysize={\textquotedbl}*{\textquotedbl} and
ucd={\textquotedbl}phys.atmol.element{\textquotedbl}.
In the case of an atom, the corresponding VAMDC returnable is {\textquotedbl}\textbf{AtomSymbol}{\textquotedbl}.
If this is a molecular line, the corresponding VAMDC returnable is  {\textquotedbl}\textbf{MoleculeChemicalName}{\textquotedbl}.\\
Example of valid values are : Fe for a iron line or CH4 for a methane line.

\subsubsection{Chemical element mass}

Exactly one field \textbf{MAY} have name={\textquotedbl}chemical\_element\_mass{\textquotedbl}, with  datatype={\textquotedbl}float{\textquotedbl} and ucd={\textquotedbl}phys.atmol.element{\textquotedbl}. The field contains the atom mass or the molecular weight of the species, to be specified in Unified Atomic Mass Unit (u). For an atom, the ccorresponding VAMDC returnable is {\textquotedbl}\textbf{AtomMass}{\textquotedbl}. If this is a molecular line, the corresponding VAMDC returnable is {\textquotedbl}\textbf{MoleculeMolecularWeight}{\textquotedbl}.

\subsubsection{InChiKey}

Exactly one field \textbf{SHOULD} have name={\textquotedbl}InChiKey{\textquotedbl}, with datatype={\textquotedbl}char{\textquotedbl}, arraysize={\textquotedbl}*{\textquotedbl} and
ucd={\textquotedbl}phys.atmol.element{\textquotedbl}, containing the inchikey \citep{Heller2015} of
the chemical element of this line. For an atom, the corresponding VAMDC returnable is  {\textquotedbl}\textbf{{AtomInchiKey}}{\textquotedbl} and {\textquotedbl}\textbf{{MoleculeInchiKey}}{\textquotedbl} for a molecular line.

\subsubsection{InChi}

Exactly one field \textbf{MAY} have name={\textquotedbl}InChi{\textquotedbl}, with
datatype={\textquotedbl}char{\textquotedbl}, arraysize={\textquotedbl}*{\textquotedbl} and
ucd={\textquotedbl}phys.atmol.element{\textquotedbl}, containing the inchi of the
chemical element for the lower level of this line. The corresponding VAMDC returnable is  {\textquotedbl}\textbf{{AtomInchi}}{\textquotedbl} for an atomic line and  {\textquotedbl}\textbf{{MoleculeInchi}}{\textquotedbl} for a molecular line.

\subsubsection{Ion charge}

Exactly one field \textbf{SHOULD} have name={\textquotedbl}ionCharge{\textquotedbl}, with
datatype={\textquotedbl}int{\textquotedbl} and ucd={\textquotedbl}phys.atmol.ionization{\textquotedbl}, containing the charge of the chemical element origin of the line. The corresponding VAMDC returnable is {\textquotedbl}\textbf{{AtomIonCharge}}{\textquotedbl} for an atomic line and {\textquotedbl}\textbf{{MoleculeIonCharge}}{\textquotedbl}  for a molecular line.

\subsubsection{Lower state description}

Exactly one field \textbf{SHOULD} have name={\textquotedbl}lower\_state\_description{\textquotedbl}, with datatype={\textquotedbl}char{\textquotedbl}, arraysize={\textquotedbl}*{\textquotedbl} and ucd={\textquotedbl}phys.atmol.level{\textquotedbl}, containing a full description of the lower level of the transition originating the line. The corresponding VAMDC returnable is  {\textquotedbl}\textbf{AtomStateDescription}{\textquotedbl} for an atomic state and {\textquotedbl}\textbf{MoleculeStateDescription}{\textquotedbl} for a molecular state.

\subsubsection{Upper state description}

Exactly one field \textbf{SHOULD} have name={\textquotedbl}upper\_state\_description{\textquotedbl}, with datatype={\textquotedbl}char{\textquotedbl}, arraysize={\textquotedbl}*{\textquotedbl} and ucd={\textquotedbl}phys.atmol.level{\textquotedbl}, containing a full description of the upper level of the transition originating the line. The corresponding VAMDC returnable is  {\textquotedbl}\textbf{AtomStateDescription}{\textquotedbl} for an atomic state and  {\textquotedbl}\textbf{MoleculeStateDescription}{\textquotedbl} for a molecular state, 

\subsubsection{Observed wavelength}

Exactly one field \textbf{MAY} have name={\textquotedbl}observed\_wavelength{\textquotedbl}, with
datatype={\textquotedbl}double{\textquotedbl} unit={\textquotedbl}m{\textquotedbl} and ucd={\textquotedbl}em.wl{\textquotedbl}, containing the observed wavelength in the vacuum of the transition originating the line in meters, as it was observed. This may be used by observational spectral line databases. The corresponding VAMDC returnable is  {\textquotedbl}\textbf{{RadTransWavelength}}{\textquotedbl}.

\subsubsection{Lower level energy}

Exactly one field \textbf{MAY} have name={\textquotedbl}lower\_level\_energy{\textquotedbl}, with
datatype={\textquotedbl}double{\textquotedbl}, unit={\textquotedbl}J{\textquotedbl} and
ucd={\textquotedbl}phys.energy;phys.atmol.level{\textquotedbl} describing the energy for the lower level of the transition which originates this line. The value must appear in Joules to unify results.
The corresponding VAMDC returnable is  {\textquotedbl}\textbf{StateEnergy}{\textquotedbl}.
It is allowed to have more than one \textbf{Line.lowerLevel.energy.value} field in different units in order to preserve the precision of the original value prior to unit conversion. If this is the case and to get backwards compatibility with already existing services, there \textbf{MUST} be one field with
name={\textquotedbl}\textbf{lower\_level\_energy}{\textquotedbl} where unit={\textquotedbl}J{\textquotedbl} in the VOTable response. Other fields with the same utype should have a different value in the unit field descriptor.

\subsubsection{Upper level energy}

Exactly one field \textbf{MAY} have corresponding, with
datatype={\textquotedbl}double{\textquotedbl}, unit={\textquotedbl}J{\textquotedbl} and
ucd={\textquotedbl}phys.energy;phys.atmol.level{\textquotedbl},
describing the energy for the upper level of the transition which originates this line. The value must appear in Joules to unify results. The VAMDC returnable is {\textquotedbl}\textbf{StateEnergy}{\textquotedbl}2

It is allowed to have more than one
\textbf{Line.upperLevel.energy.value} field in different units in order
...
name={\textquotedbl}\textbf{upper\_level\_energy}{\textquotedbl} where unit={\textquotedbl}J{\textquotedbl}
...
\paragraph{Lower level}

Exactly one field \textbf{MAY} have name={\textquotedbl}lower\_level\_configuration{\textquotedbl}, with ucd={\textquotedbl}phys.atmol.configuration{\textquotedbl}, datatype={\textquotedbl}char{\textquotedbl} and arraysize={\textquotedbl}*{\textquotedbl} describing the electron configuration of the lower level of the line. The corresponding VAMDC restrictable is  {\textquotedbl}\textbf{AtomStateConfigurationLabel}{\textquotedbl}.

\paragraph{Upper level}

Exactly one field \textbf{MAY} have name={\textquotedbl}upper\_level\_configuration{\textquotedbl}, with ucd={\textquotedbl}phys.atmol.configuration{\textquotedbl}, datatype={\textquotedbl}char{\textquotedbl} and arraysize={\textquotedbl}*{\textquotedbl} describing the electron configuration of the upper level of the line. The corresponding VAMDC restrictable is  {\textquotedbl}\textbf{AtomStateConfigurationLabel}{\textquotedbl}.
...
\paragraph{Lower state}
Exactly one field \textbf{MAY} have name={\textquotedbl}lower\_state\_quantum\_numbers{\textquotedbl} with datatype={\textquotedbl}char{\textquotedbl} and arraysize={\textquotedbl}*{\textquotedbl} describing the quantum state of the lower in a string representation. The corresponding VAMDC returnable is  {\textquotedbl}\textbf{MoleculeStateQuantumNumbers}{\textquotedbl}.

\paragraph{Upper state}
Exactly one field \textbf{MAY} have name={\textquotedbl}lower\_state\_quantum\_numbers{\textquotedbl} with datatype={\textquotedbl}char{\textquotedbl}{\textquotedbl} and arraysize={\textquotedbl}*{\textquotedbl} describing the quantum state of the lower in a string representation. The corresponding VAMDC returnable is {\textquotedbl}\textbf{MoleculeStateQuantumNumbers}{\textquotedbl}, 
...
\subsubsection{Reference DOI}

One or more fields \textbf{SHOULD} have name={\textquotedbl}reference\_doi{\textquotedbl}, with ucd={\textquotedbl}meta.ref.doi{\textquotedbl}, datatype={\textquotedbl}char{\textquotedbl} and
arraysize={\textquotedbl}*{\textquotedbl}, to specify a digital object identifier
attached to this reference. The corresponding VAMDC returnable is  {\textquotedbl}\textbf{SourceDOI}{\textquotedbl}.

\subsubsection{Reference URI}

One or more fields \textbf{SHOULD} have name={\textquotedbl}reference\_uri{\textquotedbl},
with ucd={\textquotedbl}meta.ref.uri{\textquotedbl}, datatype={\textquotedbl}char{\textquotedbl} and
arraysize={\textquotedbl}*{\textquotedbl}, to specify an http link that contains a scientific publication related to the spectral line. Since the URL will often contain metacharacters, the URL is normally enclosed in an XML CDATA section ({\textless}![CDATA[...]]{\textgreater}) or otherwise encoded to escape any embedded metacharacters. The corresponding VAMDC returnable is {\textquotedbl}\textbf{SourceURI}{\textquotedbl}.


\subsubsection{Additional references}

One or more fields \textbf{MAY} have name={\textquotedbl}reference\_additional\_uri{\textquotedbl} with ucd={\textquotedbl}meta.ref.uri{\textquotedbl}, datatype={\textquotedbl}char{\textquotedbl}, arraysize={\textquotedbl}*{\textquotedbl} and free names, to specify URLs that contains extra information related to the spectral line. Same criteria than before about the CDATA section.
The corresponding VAMDC restrictable is  {\textquotedbl}\textbf{SourceURI}{\textquotedbl}.
...
A service \textbf{SHOULD} implement the parameter \textbf{ELEMENT\_TYPE} to specify
the type of chemical elements expected in the response. The possible values are {\textquotedbl}atom{\textquotedbl} and {\textquotedbl}molecule{\textquotedbl}.
...
A service \textbf{SHOULD} implement the parameter \textbf{INCHIKEY} to test the existence of a given inchikey in the service data among the data shared by the service.
...
A service \textbf{SHOULD} implement the parameter \textbf{CHEMICAL\_ELEMENT\_NAME\_STARTSWITH}. It defines a list of characters with which the name of the chemical species must start. The search is case-sensitive.
...
A service \textbf{SHOULD} implement the parameter \textbf{CHEMICAL\_ELEMENT\_NAME\_CONTAINS}. It defines a list of characters which is contained somewhere in the name of the chemical species must start. The search is case-sensitive.
...
The stoichiometric formula is a chemical expression that indicates the quantitative composition of atoms that make up the molecule, e.g. CO2, NH3, CH4.

A service \textbf{SHOULD} implement the parameter \textbf{STOICHIOMETRIC\_FORMULA\_STARTSWITH}. It defines a list of characters with which the stoichiometric formula of the chemical species must start. The search is case-sensitive.
...
A service \textbf{SHOULD} implement the parameter \textbf{STOICHIOMETRIC\_FORMULA\_CONTAINS}. It defines a list of characters which is contained somewhere in stoichiometric formula of the chemical species. The search is case-sensitive.
...
name={\textquotedbl}chemical\_element\_name{\textquotedbl}. The
corresponding VAMDC returnable is  {\textquotedbl}\textbf{AtomSymbol}{\textquotedbl} for an atom
and {\textquotedbl}\textbf{MoleculeChemicalName}{\textquotedbl} for a molecule.

This is the conventional molecule name, e.g. CO2, NH3, Feh, Carbon, Methane ... and it may not be unique.

In both cases, datatype={\textquotedbl}char{\textquotedbl},
arraysize={\textquotedbl}*{\textquotedbl} and
ucd={\textquotedbl}phys.atmol.element{\textquotedbl}, containing the name of a
...
Exactly one field \textbf{SHOULD} have
name={\textquotedbl}chemical\_element\_type{\textquotedbl}, datatype={\textquotedbl}char{\textquotedbl},
arraysize={\textquotedbl}*{\textquotedbl}. It will describe the type of chemical element, possible values are
{"atom"} or {"molecule"}

\subsubsection{Chemical element stoichiometric formula}

Exactly one field \textbf{SHOULD} have
name={\textquotedbl}chemical\_element\_stoichiometric\_formula{\textquotedbl}, datatype={\textquotedbl}char{\textquotedbl},
arraysize={\textquotedbl}*{\textquotedbl} and
ucd={\textquotedbl}phys.atmol.element{\textquotedbl}, containing the stoichiometric formula of a chemical element for which at least one spectral line is available in the service. For an atom, the atom symbol can be used as a value.

The corresponding VAMDC returnable is {\textquotedbl}\textbf{MoleculeStoichiometricFormula}{\textquotedbl} in the case of a molecule. The value is not defined for an atom.


\subsubsection{Ion charge}

Exactly one field \textbf{SHOULD} have name={\textquotedbl}ion\_charge{\textquotedbl} with datatype={\textquotedbl}int{\textquotedbl} and ucd={\textquotedbl}phys.atmol.ionization{\textquotedbl}, containing the charge of  the considered chemical species.
The corresponding VAMDC returnable is {\textquotedbl}\textbf{{AtomIonCharge}}{\textquotedbl} for an atom and  {\textquotedbl}\textbf{{MoleculeIonCharge}}{\textquotedbl} for a molecule. 

\subsubsection{InChiKey}

Exactly one field \textbf{SHOULD} have name={\textquotedbl}inchikey{\textquotedbl}, with
datatype={\textquotedbl}char{\textquotedbl},
arraysize={\textquotedbl}*{\textquotedbl} and
ucd={\textquotedbl}phys.atmol.element{\textquotedbl}, containing the inchikey 
of the chemical element.

The corresponding VAMDC returnable is  {\textquotedbl}\textbf{{AtomInchiKey}}{\textquotedbl} for an atom and {\textquotedbl}\textbf{{MoleculeInchiKey}}{\textquotedbl} for a molecule.

\subsubsection{InChi}

Exactly one field \textbf{MAY} have name={\textquotedbl}inchi{\textquotedbl}, with datatype={\textquotedbl}char{\textquotedbl}, arraysize={\textquotedbl}*{\textquotedbl} and ucd={\textquotedbl}phys.atmol.element{\textquotedbl}, containing the inchi of the chemical element. The corresponding VAMDC returnable is  {\textquotedbl}\textbf{{AtomInchi}}{\textquotedbl} for an atom and {\textquotedbl}\textbf{{MoleculeInchi}}{\textquotedbl} for a molecule. 


\subsubsection{VOSI-capabilities }
...
\bibliography{ivoatex/ivoabib,ivoatex/docrepo,ivoatex/vamdc.bib}
