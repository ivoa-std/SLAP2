...
The \textbf{SPECIES\_MASS} parameter to specifies the minimum and maximum possible mass value 
...
At most one field \textbf{MUST} have name={"inchi"}, with
...
At most one field \textbf{MUST} have name={"transition"}, with
datatype={"char"}, unit={"*"} and ucd=\ucd{"phys.atmol.transition"},
...
A service must support all the input parameters described in this section. That means that the service must accept them as valid ones without raising an error, and the parameters must be properly used to constrain the query. If one or several parameters can not be implemented in the context of a service, an error must be raised when the parameter is received in a query(see 5.2.1 for the appropriate syntax).
...
At most one field \textbf{MUST} have name={"inchi"}, with datatype={"char"}, arraysize={"*"} and ucd=\ucd{"phys.atmol.element"}, containing the inchi of the chemical element. The corresponding VAMDC returnable is  {"\textbf{AtomInchi}"} for an atom and {"\textbf{MoleculeInchi}"} for a molecule. The NULL value is not authorized.
...
\subsubsection{number_of_elements}

At most one field \textbf{SHOULD} have name={"number_of_elements"}, with  datatype={"int"}. 
...
INCHIKEY                              & InchiKey                            & SHOULD  \\
CHEMICAL\_ELEMENT                     & AtomSymbol, MoleculeChemicalName    & SHOULD  \\
STOICHIOMETRIC\_FORMULA               & MoleculeStoichiometric\-Formula       & SHOULD  \\
NUMBER_OF_ELEMENTS                    & Not available                       & MAY  \\
