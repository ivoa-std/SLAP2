...
The \textbf{CHEMICAL\_ELEMENT} parameter defines an expression using the SQL LIKE syntax to perform
a search in the list of chemical elements provided by the service. The search is case-sensitive.

Returns carbon monoxide and carbon dioxide :
\parexample{CHEMICAL\_ELEMENT=CO\_}

Returns Cobalt atom :
\parexample{CHEMICAL\_ELEMENT=Co}

Returns all the chemical species whose formula ends with "OH" :
\parexample{CHEMICAL\_ELEMENT=\%OH}
...
the service. The NULL value is not authorized.

\subsubsection{ion\_charge}

Exactly one field \textbf{MUST} have name={"ion\_charge"} with datatype={"int"} and ucd=\ucd{"phys.atmol.element;phys.atmol.ionization"}, containing the charge of  the considered chemical species.
The corresponding VAMDC returnable is {"\textbf{{AtomIonCharge}}"} for an atom and  {"\textbf{{MoleculeIonCharge}}"} for a molecule. The NULL value is not authorized.
...
The corresponding VAMDC returnable is  {"\textbf{{AtomInchiKey}}"} for an atom and {"\textbf{{MoleculeInchiKey}}"} for a molecule. The NULL value is not authorized.

\subsubsection{inchi}

Exactly one field \textbf{MUST} have name={"inchi"}, with datatype={"char"}, arraysize={"*"} and ucd=\ucd{"phys.atmol.element"}, containing the inchi of the chemical element. The corresponding VAMDC returnable is  {"\textbf{AtomInchi}"} for an atom and {"\textbf{MoleculeInchi}"} for a molecule. The NULL value is not authorized.
...
The corresponding VAMDC returnable is {"\textbf{MoleculeStoichiometricFormula}"} in the case of a molecule. The value is not defined for an atom. The NULL value is authorized.

\subsubsection{chemical\_element\_type}
...
\subsubsection{number\_of\_elements}

At most one field \textbf{SHOULD} have name={"number\_of\_elements"}, with  datatype={"int"}. 
...
<VOTABLE version="1.3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<RESOURCE type="results">
      ...
      <FIELD ucd="em.wl" name="vacuum_wavelength_error" datatype="double" unit="m" />
			...
			<FIELD ucd="phys.atmol.element" name="inchi" datatype="char" arraysize="*" />
...
CHEMICAL\_ELEMENT    & AtomSymbol, MoleculeChemicalName    \\
INCHIKEY             & InchiKey                            \\
...
chemical\_element\_name                & AtomSymbol, MoleculeChemicalName                              & MUST    &  NO   \\
chemical\_element\_mass                & AtomMass, MoleculeMolecularWeight                             & MAY     &  YES  \\
...
\textbf{SLAP2 parameter name} & \textbf{VAMDC restrictable name}\\
\hline
ELEMENT\_TYPE                         & Not available                       \\
INCHIKEY                              & InchiKey                            \\
CHEMICAL\_ELEMENT                     & AtomSymbol, MoleculeChemicalName    \\
STOICHIOMETRIC\_FORMULA               & MoleculeStoichiometric\-Formula     \\
NUMBER\_OF\_ELEMENTS                  & Not available                       \\
...
inchikey                                    & AtomInchiKey, MoleculeInchiKey   & MUST     &  NO   \\
inchi                                       & AtomInchi, MoleculeInchi         & MUST     &  NO   \\
chemical\_element\_stoichiometric\_formula  & MoleculeStoichiometricFormula    & MUST     &  YES  \\
chemical\_element\_type                     & Not available                    & SHOULD   &  YES  \\
number\_of\_elements                        & Not available                    & SHOULD   &  YES  \\
