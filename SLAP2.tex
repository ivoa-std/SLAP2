...
This could be done by defining H as the searched SPECIES and providing a narrow WAVELENGTH interval corresponding to each lines in the serie.
...
To achieve this, the service should support queries based on relevant filtering criteria, 
...
allowing users to remove lines that are unlikely to be present under the physical conditions 
of interest.
...
Examples : 
\parexample{SPECIES=CO*}  : all species starting with CO \\
\parexample{SPECIES=*OH} : all species ending with OH \\
\parexample{SPECIES=CO[2-3]} : CO2 and CO3 \\
\parexample{SPECIES=CH3?H} : CH3OH and CH3SH \\
...
The \textbf{SPECIES} parameter would constrain the search to
...
Atom can be specified exactly by symbol (corresponding VAMDC restrictable is {\textquotedbl}AtomSymbol{\textquotedbl}) or names
like carbon or iron. 
Molecules can be specified by conventional molecular name (formula like CO2, CH4 or names like carbon dioxyde or water {\dots}) 
which might not be unique (corresponding VAMDC restrictable are {\textquotedbl}MoleculeChemicalName{\textquotedbl} and 
{\textquotedbl}MoleculeOrdinaryStructuralFormula{\textquotedbl}). 
The \{species\} endpoint is the recommended place to discover the correct syntax of a given species in a particular service, 
providing a pattern matching syntax.
...
of an ion (ucd=\ucd{phys.atmol.ionization}; 
corresponding VAMDC restrictable is {\textquotedbl}IonCharge{\textquotedbl}).
 
It will look for the ionized forms of the species specified in SPECIES
...
The RESOURCE element \textbf{SHOULD} contain an INFO with
name={\textquotedbl}request{\textquotedbl} containing the full request 
...
database may provide transitions in $cm^{-1}$, GHz or {\AA}.
...
One field MAY have name="vacuum\_wavelength\_original" with the same attributes
...
with datatype={"double"}, unit={"m"} and
ucd=\ucd{"stat.error;em.wl"}, containing the total error for the wavelength in
vacuum of the transition originating the line in meters. 

The corresponding VAMDC returnable is  {"\textbf{RadTransWavelengthAccuracy"}}.
This value may be NULL if the value is unknown.

One field MAY have name="vacuum\_wavelength\_error\_original" with the same attributes
...
name={"species\_name"}, with  datatype={"char"}, arraysize={"*"} and
...
At most one field \textbf{MAY} have name={"species\_mass"}, with  datatype={"float"} and ucd=\ucd{"phys.mass"}. 
...
datatype={"int"} and ucd=\ucd{"phys.atmol.ionization"}, containing the charge of the species origin of the line. 
The corresponding VAMDC returnable is {"\textbf{AtomIonCharge}"} for an atomic line and {"\textbf{{MoleculeIonCharge}}"} for a molecular line. This value must not be NULL.
...
One field MAY have name="lower\_energy\_original" with the same attributes
...
One field MAY have name="upper\_energy\_original" with the same attributes
...
Examples: \\
Returns the molecular hydrogen, exact match : \\
...
Returns carbon monoxide and carbon dioxide :
\parexample{SPECIES=CO*}

Returns Cobalt atom :
\parexample{SPECIES=Co}

Returns all the species whose formula ends with "OH" :
...
Returns methanol and its isotopologues (e.g. \textsuperscript{13}CH$_3$OH, CH$_3$O\textsuperscript{17}H) :
...
arraysize={"*"} and
ucd=\ucd{"phys.atmol.element"}, containing the name of a
species for which at least one spectral line is available in
the service. This value must not be NULL.

\subsubsection{ion\_charge}

Exactly one field \textbf{MUST} have name={"ion\_charge"} with datatype={"int"} and ucd=\ucd{"phys.atmol.ionization"}, 
containing the charge of  the considered chemical species. The corresponding VAMDC returnable is {"\textbf{{AtomIonCharge}}"} 
for an atom and  {"\textbf{{MoleculeIonCharge}}"} for a molecule. This value must not be NULL.

\subsubsection{inchikey}

Exactly one field \textbf{MUST} have name={"inchikey"}, with
datatype={"char"}, arraysize={"*"} and ucd=\ucd{"phys.atmol.element"}, containing the inchikey 
...
Exactly one field \textbf{MUST} have name={"inchi"}, with datatype={"char"}, arraysize={"*"} and ucd=\ucd{"phys.atmol.element"}, 
containing the inchi of the species. The corresponding VAMDC returnable is {"\textbf{AtomInchi}"} for an atom 
and {"\textbf{MoleculeInchi}"} for a molecule. This value must not be NULL.

\subsubsection{species\_stoichiometric\_formula}

Exactly one field \textbf{MUST} have
name={"species\_stoichiometric\_formula"}, datatype={"char"},
arraysize={"*"} and
ucd=\ucd{"phys.atmol.element"}, containing the stoichiometric formula of a species for which at least one spectral line is available in the service. For an atom, the atom symbol can be used as a value.

The corresponding VAMDC returnable is {"\textbf{MoleculeStoichiometricFormula}"} in the case of a molecule. The value is not defined for an atom. This value may be NULL.

\subsubsection{species\_type}

Exactly one field \textbf{SHOULD} have
name={"species\_type"}, datatype={"char"},
arraysize={"*"}. It will describe the type of species, possible values are
{"atom"} or {"molecule"}

\subsubsection{number\_of\_atoms}
At most one field \textbf{SHOULD} have name={"number\_of\_atoms"}, with  datatype={"int"}. 
...
Example of valid values are : 1 for the hydrogen atom, 3 for the water molecule.
...
\item Exactly one field \textbf{MAY} have name={"\textbf{identificationStatus}"}, with
datatype={\textquotedbl}char{\textquotedbl} and
ucd=\ucd{meta.id.cross;em.line}, describing the identification status of the line.
...
\hline
\textbf{SLAP2 parameter name} & \textbf{VAMDC restrictable name} \\
\hline
...
\hline
\textbf{SLAP2 field name} & \textbf{VAMDC returnable name} & \textbf{Requirements for compliance in SLAP2} & \textbf{Nullable} \\
\hline
...
\hline
\textbf{SLAP2 field name} & \textbf{VAMDC returnable name} & \textbf{Requirements for compliance}  & \textbf{Nullable}  \\
\hline
species\_name                     & AtomSymbol, Molecule\-Chemical\-Name    & MUST     &  NO   \\
ion\_charge                       & Atom\-Ion\-Charge, Molecule\-Ion\-Charge & MUST     &  NO   \\
inchikey                          & Atom\-Inchi\-Key, Molecule\-Inchi\-Key   & MUST     &  NO   \\
