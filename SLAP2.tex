...
A service \textbf{SHOULD} have a search parameter called
\textbf{INCHIKEY} (ucd=\ucd{phys.atmol.element}; corresponding VAMDC restrictable is {\textquotedbl}InchiKey{\textquotedbl}). This parameter would constrain the search to
...
{\textquotedbl}MoleculeMolecularWeight{\textquotedbl} for molecules{\textquotedbl}).
...
A service \textbf{MAY} implement the \textbf{ION\_CHARGE} parameter to specify the minimum and maximum charge 
...
\begin{lstlisting}{language=XML}
<INFO name="request" value="https://sesam.obspm.fr/25.02/vamdc/slap/lines?WAVELENGTH=9e-8+9.1e-8"/>
\end{lstlisting}
...
Exactly one field \textbf{SHOULD} have name={"vacuum\_wavelength\_error"},
with datatype={"double"}, unit={"m"} and
...
For an atom, the ccorresponding VAMDC returnable is {"\textbf{AtomMass}"}. 
...
At most one field \textbf{MAY} have name={"inchi"}, with
datatype={"char"}, arraysize={"*"} and
ucd=\ucd{"phys.atmol.element"}, containing the inchi of the
chemical element for the lower level of this line. The corresponding VAMDC returnable is  {"\textbf{{AtomInchi}}"} 
for an atomic line and  {"\textbf{{MoleculeInchi}}"} for a molecular line.

\subsubsection{ion\_charge}

Exactly one field \textbf{SHOULD} have name={"ion\_charge"}, with
datatype={"int"} and ucd=\ucd{"phys.atmol.ionization"}, containing the charge of the chemical element origin of the line. 
The corresponding VAMDC returnable is {"\textbf{AtomIonCharge}"} for an atomic line and {"\textbf{{MoleculeIonCharge}}"} for a molecular line.

\subsubsection{lower\_level\_description}

Exactly one field \textbf{SHOULD} have name={"lower\_level\_description"}, with datatype={"char"}, arraysize={"*"} 
and ucd=\ucd{"phys.atmol.level;meta.title"}, containing a full description of the lower level of the transition originating the line. 
The corresponding VAMDC returnable is  {"\textbf{AtomStateDescription}"} for an atomic state and {"\textbf{MoleculeStateDescription}"} 
...
Exactly one field \textbf{SHOULD} have name={"upper\_level\_description"}, with datatype={"char"}, arraysize={"*"} 
and ucd=\ucd{"phys.atmol.level;meta.title"}, containing a full description of the upper level of the transition originating the line. 
The corresponding VAMDC returnable is  {"\textbf{AtomStateDescription}"} for an atomic state and  {"\textbf{MoleculeStateDescription}"} 
for a molecular state.
...
At most one field \textbf{MAY} have name={"upper\_level\_energy"}, corresponding, with
...
At most one field \textbf{MAY} have name={"lower\_level\_configuration"}, with ucd=\ucd{"phys.atmol.configuration"}, 
datatype={"char"} and arraysize={"*"} describing the electron configuration of the lower level of the line. 
The corresponding VAMDC restrictable is  {"\textbf{AtomStateConfigurationLabel}"}.

\paragraph{upper\_level\_configuration}

At most one field \textbf{MAY} have name={"upper\_level\_configuration"}, with ucd=\ucd{"phys.atmol.configuration"}, 
datatype={"char"} and arraysize={"*"} describing the electron configuration of the upper level of the line. 
The corresponding VAMDC restrictable is  {"\textbf{AtomStateConfigurationLabel}"}.
...
to use a LaTeX-styled \citep{lamport86latex} convention to describe sub-indexes,
...
At most one field \textbf{MAY} have name={"lower\_state\_quantum\_numbers"} with datatype={"char"} and arraysize={"*"} 
describing the quantum state of the lower in a string representation.
The corresponding VAMDC returnable is  {"\textbf{MoleculeStateQuantumNumbers}"}.
...
The recommended syntax for the quantum numbers is name:nominator:denominator. Quantum Numbers are separated by the ``;'' character.
...
A service \textbf{MAY} implement the \textbf{ATOMICITY} parameter to specify the number of atoms in the returned
species.
...
\parexample{ATOMICITY=2}

Molecules containing 2 to five atoms :

\parexample{ATOMICITY=2 5}
...
arraysize={"*"} and
ucd=\ucd{"phys.atmol.element"}, containing the stoichiometric formula of a chemical element for which at least one spectral line is available in the service. For an atom, the atom symbol can be used as a value.
...
Exactly one field \textbf{SHOULD} have name={"ion\_charge"} with datatype={"int"} and ucd=\ucd{"phys.atmol.ionization"}, containing the charge of  the considered chemical species.
...
datatype={"char"},
arraysize={"*"} and
ucd=\ucd{"phys.atmol.element"}, containing the inchikey 
...
At most one field \textbf{MAY} have name={"inchi"}, with datatype={"char"}, arraysize={"*"} and ucd=\ucd{"phys.atmol.element"}, containing the inchi of the chemical element. The corresponding VAMDC returnable is  {"\textbf{AtomInchi}"} for an atom and {"\textbf{MoleculeInchi}"} for a molecule. 

\subsubsection{atomicity}

At most one field \textbf{MAY} have name={"atomicity"}, with  datatype={"int"}. 
...
ucd=\ucd{em.wl.central;meta.id.cross}, describing if the wavelength has been calculated.
...
ucd=\ucd{em.line;meta.id.cross}, describing the identification status of the line.
...
<VOTABLE version="1.3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<RESOURCE type="results">
		...
		<INFO name="request_date" value="2025-10-31T14:57:18+01:00" />
		<INFO name="service_protocol" value="ivo://ivoa.net/std/SLAP#lines-2.0" />
		<INFO name="publisher" value="sesam.obspm.fr" />
		<INFO name="last_update_date" value="2025-05-31T10:05:00+01:00" />
		<TABLE>
...
\centering
\small
\begin{tabularx}{\textwidth}{|p{4.5cm}|>{\raggedright\arraybackslash}X|p{2.5cm}|}
\hline
...
\centering
\small
\begin{tabularx}{\textwidth}{|p{5.5cm}|>{\raggedright\arraybackslash}X|p{2.3cm}|}
\hline
...
vacuum\_wavelength\_original           & RadTransWavelength                                            & MAY  \\
vacuum\_wavelength\_error              & RadTransWavelengthAccuracy                                    & MUST  \\
vacuum\_wavelength\_error\_original    & RadTransWavelengthAccuracy                                    & MUST  \\
...
einstein\_a                             & RadTransProbabilityA                                          & MAY  \\
process                                & RadTransProcess                                               & MAY  \\
lower\_level\_energy                   & StateEnergy                                                   & MAY  \\
lower\_level\_energy\_original         & StateEnergy                                                   & MAY  \\
upper\_level\_energy                   & StateEnergy                                                   & MAY  \\
upper\_level\_energy\_original         & StateEnergy                                                   & MAY  \\
lower\_level\_description              & AtomStateDescription, MoleculeStateDescription                & SHOULD  \\
upper\_level\_description              & AtomStateDescription, MoleculeStateDescription                & SHOULD  \\
lower\_level\_configuration            & AtomState\-ConfigurationLabel                                   & MAY  \\
upper\_level\_configuration            & AtomState\-ConfigurationLabel                                   & MAY  \\
lower\_level\_quantum\_numbers         & MoleculeState\-QuantumNumbers                                   & MAY  \\
upper\_level\_quantum\_numbers         & MoleculeState\-QuantumNumbers                                   & MAY  \\
...
\centering
\footnotesize
\begin{tabularx}{\textwidth}{|p{5.8cm}|>{\raggedright\arraybackslash}X|p{2cm}|}
\hline
\textbf{SLAP2 parameter name} & \textbf{VAMDC restrictable name} & \textbf{Requirements} \\
\hline
ELEMENT\_TYPE                         & Not available                       & SHOULD  \\
INCHIKEY                              & InchiKey                            & SHOULD  \\
CHEMICAL\_ELEMENT\_NAME\_\allowbreak STARTSWITH   & AtomSymbol, MoleculeChemicalName    & SHOULD  \\
CHEMICAL\_ELEMENT\_NAME\_\allowbreak CONTAINS     & AtomSymbol, MoleculeChemicalName    & SHOULD  \\
STOICHIOMETRIC\_FORMULA\_\allowbreak STARTSWITH   & MoleculeStoichiometric\-Formula       & SHOULD  \\
STOICHIOMETRIC\_FORMULA\_\allowbreak CONTAINS     & MoleculeStoichiometric\-Formula       & SHOULD  \\
ATOMICITY                             & Not available                       & MAY  \\
...
\centering
\small
\begin{tabularx}{\textwidth}{|p{5cm}|>{\raggedright\arraybackslash}X|p{2.5cm}|}
\hline
